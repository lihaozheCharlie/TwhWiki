...
The Way Here 会把这些原话和经历好好留下来，记住它们之间的联系，也坦白哪些地方还不懂你。你可以从任何一件想说的事开始：先把生活讲具体，再一起分清已经知道什么、可能怎样理解、还有什么只能由你亲自补充。一路聊下来的理解都能回到原始记录，也随时允许你说“不是这样的”。
...
仓库已经准备好一套匿名 `demo`。你不需要先交出自己的日记，就可以看看 The Way Here 如何记住一段生活、提出值得聊聊的问题，并写出一封有来路的回信。
...
脚本会自动检查环境、安装依赖并构建前后端。启动完成后，打开 <http://127.0.0.1:4321> 即可进入匿名样例；按 `Ctrl+C` 可以同时停止所有服务。工作区已经登记个人知识库时，服务会优先打开个人库，只有没有个人库时才进入 `demo`。
...
当你准备开始自己的空间时，可以直接从演示页提示或左上角入口创建。底层仍是一个独立的本地知识库，不会复制或混入演示数据，创建后会自动打开并成为后续启动的默认空间。

也可以手动在 `the-way-here.config.yaml` 中登记已有知识库目录：
...
服务会自动优先打开这个个人库；只有需要临时覆盖自动选择时才传入：
