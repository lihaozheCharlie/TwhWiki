...
来源导入清单同时保存每份可索引记录的冷启构建状态：叙事完整的材料进入可直接构建，账单报告进入待对话，语境过少的材料进入待确认。状态与发起的 Run 通过 `sourceContext` 关联；单条构建记录一个 `storedPath`，批量构建记录用户实际勾选的 `storedPaths`，因此可以跨导入批次准确回写每份记录的进度。直接构建在任务完成后标记为已处理，对话材料只有在任务产生实际 Wiki 变更后才标记为已构建。关联页来自 Run 的真实差异，不由前端猜测。用户选择“稍后再说”会持久化并停止重复展示导入确认卡，但记录仍保留在待构建筛选中。原始材料正文不承载这些应用状态，也不会被冷启流程改写。
